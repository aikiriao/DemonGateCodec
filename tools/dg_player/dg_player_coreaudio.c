...
static void DGPlayer_CoreAudioCallback(void *custom_data, AudioQueueRef queue, AudioQueueBufferRef buffer);

/* 初期化カウント */
static int32_t st_initialize_count = 0;
/* 初期化時のプレイヤーコンフィグ */
static struct DGPlayerConfig st_config = { 0, };
/* デコードしたデータのバッファ領域 */
...
void DGPlayer_Initialize(const struct DGPlayerConfig *config)
{
    ...
    AudioQueueNewOutput(&format,
            DGPlayer_CoreAudioCallback, NULL, CFRunLoopGetCurrent(), kCFRunLoopCommonModes, 0, &queue);
    ...
    for (i = 0; i < NUM_BUFFERS; i++) {
        ...
        /* 一発目のデータを出力 */
        DGPlayer_CoreAudioCallback(NULL, queue, buffers[i]);
    }
