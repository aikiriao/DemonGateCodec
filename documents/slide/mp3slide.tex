...
\begin{frame}[c]
    \frametitle{コサイン変調フィルタバンク}
    $h_{k}[n]$の伝達関数を求める．回転因子$W_{2M} = \exp\left(-j\frac{\pi}{M}\right)$を用いると
    \scriptsize
    \begin{align*}
        &\cos\left[ \frac{\pi}{M} \left( k + \frac{1}{2} \right) \left( n - \frac{L - 1}{2} + \theta_{k} \right) \right] \\
        &= \frac{1}{2} \left\{ \exp\left[ j \left\{  \frac{\pi}{M} \left( k + \frac{1}{2} \right) \left( n - \frac{L - 1}{2} + \theta_{k} \right) \right\} \right] + \exp\left[-j \left\{  \frac{\pi}{M} \left( k + \frac{1}{2} \right) \left( n - \frac{L - 1}{2} + \theta_{k} \right) \right\} \right] \right\} \\
    ...
    \end{align*}
    \normalsize
    これを\eqref{eq:cos_modulated_analysis_filter}式に代入すると伝達関数は，
    \scriptsize
    \begin{align*}
