...
class PsycoAcousticsModelII:
    ...
    def __init__(self, sampling_frequency):
        ...
        # 窓関数計算
        self.LONG_WINDOW = [\
                0.5 * (1.0 - np.cos(2.0 * np.pi * (i - 0.5) / self.LONG_WINDOW_SIZE))\
                for i in range(self.LONG_WINDOW_SIZE)]
        self.SHORT_WINDOW = [\
                0.5 * (1.0 - np.cos(2.0 * np.pi * (i - 0.5) / self.SHORT_WINDOW_SIZE))\
                for i in range(self.SHORT_WINDOW_SIZE)]

        # 聴覚データ取得
        self.PARTITION_LONG = self.PARTITION_DATA[f'long{self.SAMPLING_FREQUENCY}']
        self.PARTITION_SHORT = self.PARTITION_DATA[f'short{self.SAMPLING_FREQUENCY}']
        self.PSYCO_LONG = self.PSYCO_DATA[f'long{self.SAMPLING_FREQUENCY}']
        self.PSYCO_SHORT = self.PSYCO_DATA[f'short{self.SAMPLING_FREQUENCY}']

        # 広がり関数計算
        self.SPREADING_FUNCTION_LONG = self._compute_spreading_function(self.PARTITION_LONG)
        self.SPREADING_FUNCTION_SHORT = self._compute_spreading_function(self.PARTITION_SHORT)

        # 分割インデックス作成
        self.PARTITION_INDEX_LONG\
            = self._compute_partition_index(self.PARTITION_LONG, self.LONG_WINDOW_SIZE)
        self.PARTITION_INDEX_SHORT\
            = self._compute_partition_index(self.PARTITION_SHORT, self.SHORT_WINDOW_SIZE)

        # ショートブロックのSNRと閾値作成
        # パーティション個数以降は0初期化されている
        self.SNR_SHORT = np.zeros(self.NUM_CRITICAL_BANDS_SHORT)
        self.QTHR_SHORT = np.zeros(self.NUM_CRITICAL_BANDS_SHORT)
        for b, part in enumerate(self.PARTITION_SHORT):
            self.SNR_SHORT[b] = part['SNR']
            self.QTHR_SHORT[b] = part['qthr']

        self.prev_wl = np.zeros(self.LONG_WINDOW_SIZE, dtype=complex)
        self.prevprev_wl = np.zeros(self.LONG_WINDOW_SIZE, dtype=complex)

        self.prev_nb = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        self.prevprev_nb = np.zeros(self.NUM_CRITICAL_BANDS_LONG)

        self.prev_block_type = 'NORMAL'

        self.ratio_long = np.zeros(len(self.PSYCO_LONG))
        self.ratio_short = np.zeros((3, len(self.PSYCO_SHORT)))
    ...
    def _compute_fft(self, analyze_frame):
        ...
        spec_short : list of ndarray
            FFT結果（ショートブロック）
        '''
        spec_long = self._dist10fft(analyze_frame * self.LONG_WINDOW)
        spec_short = []
        for i in range(3):
            short_frame = analyze_frame[\
                128 * i + self.SHORT_WINDOW_SIZE + np.arange(self.SHORT_WINDOW_SIZE)].copy()
            spec_short.append(self._dist10fft(short_frame * self.SHORT_WINDOW))
        return spec_long, spec_short

    def _compute_spreading_function(self, partition):
        '''
        ...
        for i in range(part_max):
            for j in range(part_max):
                ...
                # dBを振幅スケールに直す
                # 最小値：規格とdist10は-60 本では-100
                sfunc[i][j] = 10.0 ** ((x_ij + t_y) / 10) if t_y >= -60 else 0.0
        return sfunc
    ...
    def _compute_unpredictability(self, wl, ws, prev_wl, prevprev_wl):
        '''
        ...
        cw : ndarray
            Unpredictability
        '''
        cw = np.zeros(self.LONG_WINDOW_SIZE // 2 + 1)
        # 振幅・位相の直線予測結果
    ...
    def _compute_energey_per_partition(self, cw, energy_long, energy_short):
        ...
        '''
        eb_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        cb_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        eb_short = np.zeros((3, self.NUM_CRITICAL_BANDS_SHORT))

        for j in range(self.LONG_WINDOW_SIZE // 2 + 1):
            tp = self.PARTITION_INDEX_LONG[j]
            if tp >= 0:
        ...
        for sblock in range(3):
            for j in range(self.SHORT_WINDOW_SIZE // 2 + 1):
                eb_short[sblock][self.PARTITION_INDEX_SHORT[j]] += energy_short[sblock][j]
    ...
    def _convolve_with_spreading_function(self, eb_long, cb_long, eb_short):
        ...
        ecb_short : ndarray
            広がり関数を畳みこんだeb_short
        '''
        ecb_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        ctb_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        ecb_short = np.zeros((3, self.NUM_CRITICAL_BANDS_SHORT))
        for b in range(self.NUM_CRITICAL_BANDS_LONG):
            for k in range(self.NUM_CRITICAL_BANDS_LONG):
                ecb_long[b] += self.SPREADING_FUNCTION_LONG[b][k] * eb_long[k]
                ctb_long[b] += self.SPREADING_FUNCTION_LONG[b][k] * cb_long[k]
        for sblock in range(3):
            for b in range(self.NUM_CRITICAL_BANDS_SHORT):
                for k in range(self.NUM_CRITICAL_BANDS_SHORT):
                    # BUG?: longパーティション使ってるのバグでは？
                    ecb_short[sblock][b] += self.SPREADING_FUNCTION_LONG[b][k] * eb_short[sblock][k]
    ...
    def _compute_permissive_noise_level(self, ecb_long, ctb_long, ecb_short):
        ...
        nb_short : ndarray
            許容ノイズレベル（ショートブロック）
        '''
        nb_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        nb_short = np.zeros((3, self.NUM_CRITICAL_BANDS_SHORT))
        for b in range(self.NUM_CRITICAL_BANDS_LONG):
            cbb = 0.0
            if ecb_long[b] != 0.0:
            ...
            # minvalは高域にいくほど減少
            snr = max(self.PARTITION_LONG[b]['minval'], 29.0 * tbb + 6.0 * (1.0 - tbb))
            # normにより正規化・オフセットをエネルギーに戻して乗算
            # normは畳み込みにより増えたエネルギーを補正するためのゲイン
            # 1.0 / np.sum(self.SPREADING_FUNCTION_LONG[b]) で計算できる
            nb_long[b] = self.PARTITION_LONG[b]['norm'] * ecb_long[b] * 10.0 ** (-snr / 10.0)
        for sblock in range(3):
            for b in range(self.NUM_CRITICAL_BANDS_SHORT):
                nb_short[sblock][b] = ecb_short[sblock][b]\
    ...
    def _compute_perceptual_threshold(self, nb_long, prev_nb, prevprev_nb, nb_short):
        ...
        thr_short : ndarray
            聴覚しきい値（ショートブロック）
        '''
        thr_long = np.zeros(self.NUM_CRITICAL_BANDS_LONG)
        thr_short = np.zeros((3, self.NUM_CRITICAL_BANDS_SHORT))
        for b in range(self.NUM_CRITICAL_BANDS_LONG):
            ...
            thr_long[b] = max(thr_long[b], self.PARTITION_LONG[b]['qthr'])
        for sblock in range(3):
            for b in range(self.NUM_CRITICAL_BANDS_SHORT):
    ...
    def _compute_perceptual_entropy(self, eb_long, thr_long):
        ...
        for b in range(self.NUM_CRITICAL_BANDS_LONG):
            ...
            bug_lines = self.PARTITION_LONG[b]['#lines']
            if b < len(self.PARTITION_SHORT):
                bug_lines = self.PARTITION_SHORT[b]['#lines']
    ...
    def _plot_analyze_result(self, energy_long, eb_long, ecb_long, nb_long, thr_long):
        '''
        分析結果のプロット
        '''
        freqs = np.linspace(0, self.SAMPLING_FREQUENCY / 2.0 + 1, len(self.PARTITION_INDEX_LONG))
        eb_long_spec = [eb_long[self.PARTITION_INDEX_LONG[i]]\
            for i in range(len(self.PARTITION_INDEX_LONG))]
        etb_long_spec = [ecb_long[self.PARTITION_INDEX_LONG[i]]\
            for i in range(len(self.PARTITION_INDEX_LONG))]
        nb_long_spec = [nb_long[self.PARTITION_INDEX_LONG[i]]\
            for i in range(len(self.PARTITION_INDEX_LONG))]
        thr_long_spec = [thr_long[self.PARTITION_INDEX_LONG[i]]\
            for i in range(len(self.PARTITION_INDEX_LONG))]
        plt.cla()
        ...
        plt.grid()
        plt.plot(freqs, 10 * np.log10(energy_long[:len(self.PARTITION_INDEX_LONG)]), label='energy')
        plt.legend()
        plt.tight_layout()
    ...
    def _compute_psyco_model_II_core(self,
        ...
        '''
        # FFT
        w_long, w_short = self._compute_fft(frame)

        # Unpredictability計算
        cw = self._compute_unpredictability(w_long, w_short, prev_wl, prevprev_wl)
        ...
        # パーティションごとのエネルギー計算
        eb_long, cb_long, eb_short\
            = self._compute_energey_per_partition(cw, energy_long, energy_short)

        # 広がり関数(Spreading Function)と畳み込み
        ecb_long, ctb_long, ecb_short\
            = self._convolve_with_spreading_function(eb_long, cb_long, eb_short)

        # ノイズ許容レベルの計算
        nb_long, nb_short\
            = self._compute_permissive_noise_level(ecb_long, ctb_long, ecb_short)
    ...
    def compute_psyco_model_II(self, frame):
        ...
        '''
        # 聴覚モデル計算
        w_long, nb_long, pe, block_type, ratio\
            = self._compute_psyco_model_II_core(
                frame, self.prev_wl, self.prevprev_wl,
                self.prev_nb, self.prevprev_nb, self.prev_block_type)

        # ブロックタイプの読み替え
        # ショートブロックにつなげるために前のタイプを切り替える
        if block_type == 'SHORT':
            if self.prev_block_type == 'NORMAL':
                self.prev_block_type = 'START'
            elif self.prev_block_type == 'STOP':
                self.prev_block_type = 'SHORT'
            # しきい値比の更新
            for sblock in range(3):
                self.ratio_short[sblock] = ratio[sblock]
        else:
            # しきい値比の更新
            self.ratio_long = ratio

        # 状態更新
        self.prevprev_wl = self.prev_wl
        self.prev_wl = w_long
        self.prevprev_nb = self.prev_nb
        self.prev_nb = nb_long
        ret_prev_block_type = self.prev_block_type
        self.prev_block_type = block_type

        return pe, ret_prev_block_type, ratio

def get_frame(model, data, gr_index):
    '''
    ...
    new_frame : ndarray
        信号フレーム
    '''
    new_frame = np.zeros(model.LONG_WINDOW_SIZE)
    for i in range(model.LONG_WINDOW_SIZE):
        smpl = model.NUM_GRANULE_SAMPLES * gr_index - 768 + i
        if smpl >= 0:
...
def print_result(model, count, pe, block_type):
    ...
    print(f'{pe:.3f}')
    if block_type != 'SHORT':
        for sb in range(len(model.PSYCO_LONG)):
            print(f'{model.ratio_long[sb]:.3f} ', end='')
        print('')
    else:
        for sblock in range(3):
            for sb in range(len(model.PSYCO_SHORT)):
                print(f'{model.ratio_short[sblock][sb]:.3f} ', end='')
            print('')
...
if __name__ == '__main__':
    ...
    for gr in range(NUM_SAMPLES // model[0].NUM_GRANULE_SAMPLES):
        for ch in range(NUM_CHANNELS):
            ...
            # 結果出力
            print_result(model[ch], count, pe, block_type)
