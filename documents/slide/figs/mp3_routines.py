...
def plot_window_functions():
    ...
    ax.set_title('start', fontsize=22)
    x = np.arange(0, 18)
    ax.scatter(x, np.sin(np.pi / 36 * (x + 1/2)), color=COLOR, marker='x')
    ...
    ax.set_title('short', fontsize=22)
    x = np.arange(0, 12)
    ax.scatter(x, np.sin(np.pi / 12 * (x + 1/2)), color=COLOR, marker='x')
    x = np.arange(-0.5, 11.6, 0.1)
    ax.plot(x, np.sin(np.pi / 12 * (x + 1/2)), color=COLOR, linewidth=0.5)
    x = np.arange(12, 24)
    ...
    x = np.arange(24, 36)
    ax.scatter(x, np.sin(np.pi / 12 * (x - 24 + 1/2)), color=COLOR, marker='x')
    x = np.arange(23.5, 35.59, 0.1)
    ax.plot(x, np.sin(np.pi / 12 * (x - 24 + 1/2)), color=COLOR, linewidth=0.5)
    ax.set_ylim(-0.1, 1.1)
    ax.set_xticks(np.arange(0, 36, 5))
    ax.tick_params(labelsize=15)
    ax.grid()
